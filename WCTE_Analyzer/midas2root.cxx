  ...
  bool ProcessMidasEvent(TDataContainer& dataContainer){
    ...
    if(fEventBuilder->CheckEndOfSpill(dataContainer)){
      ...
      if(fEventBuilder->GetNumberOfMissedFragments()==0){
        ...
        // Fill tree with all full events
        std::vector<WCTE_Event> wcteEvents = fEventBuilder->GetCompletedEvents();
        for(unsigned int i = 0; i < wcteEvents.size(); i++){
          //std::cout << "filling event " << i << std::endl;
          for (unsigned int id=0; id<dNames.size(); id++) {
            eventNumber[id] = wcteEvents[i].GetEventNumber();
            spillNumber[id] = wcteEvents[i].GetSpillNumber();
            timeStamp[id] = wcteEvents[i].GetTimeStamp(id);
            triggerTime[id]   = wcteEvents[i].GetTriggerTime(id);
            for (unsigned int ch=0; ch<8; ch++) {
              waveforms[id][ch].clear();
              waveforms[id][ch] = wcteEvents[i].GetDigitizerWaveform(id,ch);
            }
            fTree[id]->Fill();
          }
        }
      ...
      // delete all full events and print spill summary
      fEventBuilder->ClearCompletedEventsVector();
